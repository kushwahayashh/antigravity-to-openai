...
import { resolveThoughtSignature, storeThoughtSignature } from "./cache";
import {
...
function scanAndCacheSignatures(response: any): void {
    ...
    for (const cand of candidates) {
      if (cand?.content?.parts) {
        for (const part of cand.content.parts) {
          if (part?.functionCall?.id && part.functionCall.thoughtSignature) {
            storeThoughtSignature(part.functionCall.id, part.functionCall.thoughtSignature);
          }
        }
      }
    }
    ...
    .map((line) => {
      ...
      try {
        ...
        if (parsed.response !== undefined) {
          ...
          scanAndCacheSignatures(transformed);
      ...
      if (isWrapped) {
        ...
        if (normalizedThinking) {
          if (rawGenerationConfig) {
            ...
            // Adjust maxOutputTokens to account for thinking budget
            const budget = normalizedThinking.thinkingBudget ?? 0;
            const currentMax = typeof rawGenerationConfig.maxOutputTokens === "number" ? rawGenerationConfig.maxOutputTokens : 0;
            if (budget > 0 && (!currentMax || currentMax <= budget)) {
              rawGenerationConfig.maxOutputTokens = budget + 4000;
            }
            ...
            requestPayload.generationConfig = { 
              ...
              ...(budget > 0 ? { maxOutputTokens: budget + 4000 } : {})
            };
          }
        ...
        // Fix: Antigravity GenerateContentRequest does not allow "stream" or "model" fields in the body
        ...
        // For ALL models, strip reasoning/thinking text from history to prevent "Prompt too long" errors.
        // The thinking content was already shown to the user, no need to send it back to the model.
          ...
          requestPayload.contents = requestPayload.contents.map((content: any) => {
            ...
            const cleanedParts = content.parts
              .filter((part: any) => {
                ...
                // For non-Claude models, completely remove reasoning blocks without signatures
                if (!isClaudeModel) {
                  if (part.type === "reasoning" || part.thought === true || part.type === "thinking") {
                    // Keep only if it has a signature (for Gemini thinking models)
                    return Boolean(part.thoughtSignature || part.signature);
                  }
                }
                return true;
              })
              .map((part: any) => {
                ...
                // Strip text content from reasoning blocks, keep only signature
                if (part.type === "reasoning" || part.thought === true || part.type === "thinking") {
                  if (part.thoughtSignature) {
                    return { thought: true, thoughtSignature: part.thoughtSignature, text: "" };
                  }
                  if (part.signature) {
                    return { type: part.type, signature: part.signature, thinking: "" };
                  }
                }
                // Also strip reasoning_content field if present
                if (part.reasoning_content) {
                  const { reasoning_content, ...rest } = part;
                  return rest;
                }
                return part;
              });

            // Remove reasoning_content from content level too
            const { reasoning_content, ...cleanedContent } = content;
            return { ...cleanedContent, parts: cleanedParts };
          });
        }

        // For all models, inject cached thought signatures into function calls if they have IDs.
        if (Array.isArray(requestPayload.contents)) {
          requestPayload.contents = requestPayload.contents.map((content: any) => {
            if (!content || !Array.isArray(content.parts)) {
              return content;
            ...
            const newParts = content.parts.map((part: any) => {
              if (part && typeof part === "object" && part.functionCall && part.functionCall.id) {
                const sig = resolveThoughtSignature(part.functionCall.id);
                if (sig) {
                  return {
                    ...part,
                    functionCall: { ...part.functionCall, thoughtSignature: sig },
                  };
                }
              }
              return part;
