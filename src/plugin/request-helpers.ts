...
const ANTIGRAVITY_PREVIEW_LINK = "https://goo.gle/enable-preview-features";
 // TODO: Update to Antigravity link if available
 ...
 * For signed thinking blocks, we strip the text content but keep the signature
 * to satisfy the API requirement without bloating the prompt.
 */
    ...
    const filteredParts = content.parts
      .filter((part: any) => {
        ...
        if (isThinkingPart(part)) {
          // Only keep thinking blocks that have valid signatures
      ...
      .map((part: any) => {
        ...
        // For signed thinking blocks, strip the text content to reduce prompt size
        // The signature alone is sufficient for the API
        if (isThinkingPart(part) && hasValidSignature(part)) {
          const stripped: any = {};
          if (part.thought === true) {
            stripped.thought = true;
            stripped.thoughtSignature = part.thoughtSignature;
            stripped.text = ""; // Empty text, signature is what matters
          } else {
            stripped.type = part.type;
            stripped.signature = part.signature;
            stripped.thinking = ""; // Empty thinking content
          }
          return stripped;
        }
        return part;
...
function transformGeminiCandidate(candidate: any): any {
  ...
  const transformedParts = content.parts.map((part: any) => {
    ...
    if (part.type === "thinking") {
      ...
      if (part.signature) {
        pendingThoughtSignature = part.signature;
      }
      return {
        ...part,
        type: "reasoning",
    ...
    if (part.functionCall) {
      ...
      if (!call.id) {
        call.id = `gen-call-${crypto.randomUUID().slice(0, 8)}`;
      }
      if (pendingThoughtSignature) {
        call.thoughtSignature = pendingThoughtSignature;
      }
